...
def test_on_job_hunt_scenario():
    """Test the JobHunterNegotiator on the job_hunt_target scenario."""
    ...
    print("\n===== Testing JobHunterNegotiator as center agent (employer) =====")
    ...
    for i, edge_agents in enumerate(edge_combinations):
        ...
        results = run_session(
            scenario=scenario,
            center_type=JobHunterNegotiator,
    ...
    print("\n===== Testing JobHunterNegotiator as edge agent (employee) =====")
    ...
    for i, center_agent in enumerate(center_agents):
        ...
        # Place JobHunterNegotiator as the first edge agent, with a mix of other agents
        results = run_session(
            scenario=scenario,
            center_type=center_agent,
            edge_types=[JobHunterNegotiator, Boulware2025, Linear2025, Random2025],
            nsteps=10,
        )
        print(f"Center utility: {results.center_utility}")
        print(f"Edge Utilities: {results.edge_utilities}")
        print(f"Agreements: {results.agreements}")
        print(f"JobHunterNegotiator is edge 0 with utility: {results.edge_utilities[0]}")

    print("\n===== Comparing all agents as center (employer) =====")

    # Compare all agents in center role
    all_agents = [JobHunterNegotiator, Boulware2025, Linear2025, Random2025]
